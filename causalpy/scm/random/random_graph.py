...
class RandomSCM(SCM):
    def __init__(
        self,
        nr_nodes: int,
        assignment_functions: Optional[Collection[Type[Assignment]]],
            ...
            assignment_map.update(
                {
                    node: (
                        parents,
                        rs.choice(assignment_functions, seed).random_factory(
                            len(parents)
                        ),
        ...
        super().__init__(assignment_map=assignment_map, variable_tex_names=plot_names)
        if seed is not None:
            self.reseed(seed)
