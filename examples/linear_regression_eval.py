...
def evaluate(
    ...
    y_reg_hat, residuals_hat, beta_hat = regression_analytically(
        x_with_dummy, y_hat
    )
    y_reg, residuals, beta = regression_analytically(
        x_with_dummy, y
    )
    ...
    print(
        ...
        beta[0],
        "+",
        " + ".join([f"{beta.flatten()[i+1]} * {x_vars[i]}" for i in range(len(x_vars))]),
    )
