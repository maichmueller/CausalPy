...
def run_scenario(
    params, sample_size, scenario, step, device=None, **kwargs,
):
    seed = 0
    np.random.seed(seed)
    # print(coeffs)
...
if __name__ == "__main__":
    ...
    linearity_settings = [
        {"nr_layers": 1, "nr_blocks": 10, "nr_hidden": 128, "strength": 0},
        {"nr_layers": 1, "nr_blocks": 10, "nr_hidden": 128, "strength": 0.5},
    ...
    for scenario, params in zip(scenarios, linearity_settings):
        for step in range(steps):
            name = f"{modelclass}_{test_name}_scenario-{scenario}_step-{step + 1}_ss_{sample_size(step)}"
            results[name] = (
                run_scenario(params, sample_size(step), scenario, step, device=device,)
                .to_frame()
                .T
            )
